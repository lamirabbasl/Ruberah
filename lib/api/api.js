...
export function getProfilePhotoUrl(userId) {
  const imageUrl = `http://188.121.100.138/api/users/secure/profile-photo/${userId}/`;
  const token = getToken(); // from your token utils
  return `/api/proxy/image?url=${encodeURIComponent(
    imageUrl
  )}&token=${encodeURIComponent(token || "")}`;
}

export function getChildWithParentPhotoUrl(childId) {
  const imageUrl = `http://188.121.100.138/api/children/${childId}/photo/photo_with_parent`;
  const token = getToken(); // from your token utils
  return `/api/proxy/image?url=${encodeURIComponent(
    imageUrl
  )}&token=${encodeURIComponent(token || "")}`;
...
export function getChildPhotoUrl(childId) {
  const imageUrl = `http://188.121.100.138/api/children/${childId}/photo/photo_child`;
  const token = getToken(); // from your token utils
  return `/api/proxy/image?url=${encodeURIComponent(
    imageUrl
  )}&token=${encodeURIComponent(token || "")}`;
...
export function getInstallmentReceiptImage(installmentId) {
  const imageUrl = `http://188.121.100.138/api/courses/installments/${installmentId}/receipt-image/`;
  const token = getToken(); // from your token utils
  return `/api/proxy/image?url=${encodeURIComponent(
    imageUrl
  )}&token=${encodeURIComponent(token || "")}`;
...
export function getInstallmentReceiptImageAdmin(installmentId) {
  const imageUrl = `http://188.121.100.138/api/courses/manage/installments/${installmentId}/receipt-image/`;
  const token = getToken(); // from your token utils
  return `/api/proxy/image?url=${encodeURIComponent(
    imageUrl
  )}&token=${encodeURIComponent(token || "")}`;
...
export function getReceiptImageAdmin(installmentId) {
  const imageUrl = `http://188.121.100.138/api/courses/manage/registrations/${installmentId}/receipt-image/`;
  const token = getToken(); // from your token utils
  return `/api/proxy/image?url=${encodeURIComponent(
    imageUrl
  )}&token=${encodeURIComponent(token || "")}`;
...
export async function getRegistrationsAdmin() {
  try {
    const res = await api.get("courses/manage/registrations/");
    return res.data;
  } catch (error) {
    console.error("Error fetching registrations:", error.message);
    throw error;
...
export async function uploadInstallmentPayment(installmentId, file) {
  try {
    ...
    const res = await api.patch(`courses/installments/${installmentId}/upload-receipt/`, formData, {
...
export async function verifyPhoneValidationCode(phone_number, code) {
  try {
...
export async function registerUser({ username, phone_number, password, registration_code }) {
  try {
