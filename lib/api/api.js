...
export function getProfilePhotoUrl(userId) {
  const imageUrl = `http://127.0.0.1:8000/api/users/secure/profile-photo/${userId}/`;
...
export async function getBatchById(batchId) {
  ...
  export async function uploadInstallmentPayment(installmentId, file) {
    try {
      ...
      formData.append("receipt_image", file, file.name);

      const res = await api.post(`courses/installments/${installmentId}/upload-receipt/`, formData);
      return res.data;
    } catch (error) {
