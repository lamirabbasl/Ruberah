...
export async function getAdminChildren(page = 1) {
  try {
    const res = await api.get(`children/manage/childs/?page=${page}`);
    return res.data;
  } catch (error) {
...
export async function getRegistrationsAdmin(page = 1, search = '') {
  try {
    const searchQueryParam = search ? `&search=${search}` : '';
    const res = await api.get(`courses/manage/registrations/?page=${page}${searchQueryParam}`);
    return res.data;
  } catch (error) {
    console.error("Error fetching registrations:", error.message);
    throw error;
