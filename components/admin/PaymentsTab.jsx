...
const PaymentsTab = ({ batchId = null }) => {
  ...
  const handleApproveInstallmentPayment = async (instId, regId) => {
    try {
      ...
      toast.success("قسط با موفقیت تایید شد");
      ...
      toast.error("خطا در تایید قسط");
    }
  };

  const handleApproveSignup = async (regId) => {
    if (approvingSignupIds.has(regId)) return;
    setApprovingSignupIds((prev) => new Set(prev).add(regId));
    try {
      const response = await approveUserSignup(regId);
      setRegistrations((prev) =>
        prev.map((r) => (r.id === regId ? { ...r, approval_status: "approved" } : r))
      );
      toast.success(response.message || "ثبت نام با موفقیت تایید شد");
    } catch (error) {
      console.error("Error approving signup:", error);
      toast.error(error.message || "خطا در تایید ثبت نام");
    } finally {
      setApprovingSignupIds((prev) => {
        const newSet = new Set(prev);
        newSet.delete(regId);
        return newSet;
      });
  ...
  const handleRejectInstallment = async (instId, regId) => {
    ...
    try {
      ...
      toast.success("قسط رد شد");
    } catch (error) {
      console.error("Error rejecting installment:", error);
      toast.error("خطا در رد قسط");
    } finally {
