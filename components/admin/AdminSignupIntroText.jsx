...
function IntroTextManager() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.text.trim()) {
      const errorMessage = "لطفاً متن را وارد کنید.";
      toast.error(errorMessage);
      setFormError(errorMessage);
      return;
    }
    if (!formData.order || formData.order < 1) {
      const errorMessage = "لطفاً ترتیب معتبر (بزرگ‌تر از صفر) وارد کنید.";
      toast.error(errorMessage);
      setFormError(errorMessage);
      return;
    }
    try {
      ...
      const successMessage = response.data?.message || (editId ? "متن با موفقیت به‌روزرسانی شد." : "متن با موفقیت ایجاد شد.");
      ...
      >
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 sm:mb-6">
          ...
          <motion.button
            ...
            className="mt-4 sm:mt-0 bg-indigo-600 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg shadow-md hover:bg-indigo-700 transition-all duration-200 text-sm sm:text-base font-medium"
          >
            افزودن متن جدید
          </motion.button>
        </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4" dir="rtl">
                ...
                <div className="flex justify-between space-x-2 sm:space-x-3 space-x-reverse pt-3 sm:pt-4">
                  ...
                  <motion.button
                    ...
                    type="submit"
                    disabled={loading}
                    className={`px-3 sm:px-5 py-2 rounded-lg text-white ${
